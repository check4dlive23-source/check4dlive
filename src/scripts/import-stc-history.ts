/**
 * Import Sandakan STC 4D history → draw_results_v2
 * Source: check4d.org/past-results
 * Run: npm run import:stc
 */
...
async function main() {
  ...
  console.log(`[stc] Import ${from} → ${to} via check4d.org`);
  let upserted = 0;
  let skip = 0;
  const errors: string[] = [];
  let n = 0;
  ...
  for (const date of iterDates(from, to)) {
    ...
    n++;
    try {
      const row = await fetchCheck4dOperatorDraw(date, "sandakan");
      if (!row) skip += 1;
      else {
    ...
    if (n % 25 === 0) {
      console.log(`[stc] ${n} draw days, upserted=${upserted}`);
    }
    if (delay > 0) await sleep(delay);
  }

  console.log(`[stc] Done. upserted=${upserted} skip=${skip} errors=${errors.length}`);
  if (errors.length) {

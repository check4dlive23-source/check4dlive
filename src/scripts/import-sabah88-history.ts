/**
 * Import Sabah 88 4D history → draw_results_v2
 * Source: check4d.org/past-results
 * Run: npm run import:sabah88
 */
import { loadEnvLocal } from "@/lib/script-env";
import { createServerClient } from "@/lib/supabase/server";
import { parseHistoryImportArgs, sleep, iterDates, isMytDrawDay } from "@/lib/ingest/import-args";
...
async function main() {
  ...
  console.log(`[sabah88] Import ${from} → ${to} via check4d.org`);
  let upserted = 0;
  let skip = 0;
  const errors: string[] = [];
  let n = 0;
  ...
  for (const date of iterDates(from, to)) {
    ...
    n++;
    try {
      const row = await fetchCheck4dOperatorDraw(date, "sabah");
      if (!row) skip += 1;
      else {
    ...
    if (n % 25 === 0) {
      console.log(`[sabah88] ${n} draw days, upserted=${upserted}`);
    }
    if (delay > 0) await sleep(delay);
  }

  console.log(`[sabah88] Done. upserted=${upserted} skip=${skip} errors=${errors.length}`);
  if (errors.length) {
